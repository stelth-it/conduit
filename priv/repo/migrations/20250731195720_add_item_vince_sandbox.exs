...
defmodule Conduit.Repo.Migrations.AddItemVinceSandbox do
  use Ecto.Migration
  
  Conduit.Repo.create_schema_if_absent("vince_sandbox")

  def change do
    create table("item", prefix: "vince_sandbox", primary_key: false) do
...
add :purchase_tax_code_value, :string, comment: "the id of the associated purchase_tax_code entry"
...
add :expense_account_value, :string, comment: "the id of the associated expense_account entry"
...
add :tax_classification_name, :string, comment: "the human friendly tax_classification name."
add :tax_classification_value, :string, comment: "the id of the associated tax_classification entry"
    timestamps()
    end
  end
end
